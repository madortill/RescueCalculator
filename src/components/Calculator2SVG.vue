<template>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 451.62 754">

      <g id="Layer_1" data-name="Layer 1">
        <rect width="451.62" height="754" />
      </g>
      <g id="Layer_3" data-name="Layer 3">
        <ellipse class="cls-1" cx="164.37" cy="589.51" rx="41.16" ry="39.8" />
        <text class="cls-4" transform="translate(152.14 601.49) scale(.97 1)">
          <tspan x="0" y="0">2</tspan>
        </text>
        <ellipse class="cls-1" cx="163.65" cy="480.23" rx="41.16" ry="39.8" />
        <text class="cls-4" transform="translate(151.42 497.16) scale(.97 1)">
          <tspan x="0" y="0">5</tspan>
        </text>
        <ellipse class="cls-1" cx="166.02" cy="372.21" rx="41.16" ry="39.8" />
        <text class="cls-4" transform="translate(153.79 387.4) scale(.97 1)">
          <tspan x="0" y="0">8</tspan>
        </text>
        <ellipse class="cls-1" cx="57.64" cy="589.63" rx="41.16" ry="39.8" />
        <text class="cls-4" transform="translate(45.41 602.7) scale(.97 1)">
          <tspan x="0" y="0">1</tspan>
        </text>
        <ellipse class="cls-1" cx="57.38" cy="480.83" rx="41.16" ry="39.8" />
        <text class="cls-4" transform="translate(45.15 495.63) scale(.97 1)">
          <tspan x="0" y="0">4</tspan>
        </text>
        <ellipse class="cls-1" cx="57.51" cy="370.38" rx="41.16" ry="39.8" />
        <text class="cls-4" transform="translate(45.28 387.26) scale(.97 1)">
          <tspan x="0" y="0">7</tspan>
        </text>
        <rect class="cls-1" x="17.62" y="658.76" width="296" height="74.43" rx="28.45" ry="28.45" />
        <text class="cls-4" transform="translate(45.57 712.34) scale(.98 1)">
          <tspan x="0" y="0">0</tspan>
        </text>
        <ellipse class="cls-1" cx="270.76" cy="587.73" rx="41.16" ry="39.8" />
        <text class="cls-4" transform="translate(258.53 602.21) scale(.97 1)">
          <tspan x="0" y="0">3</tspan>
        </text>
        <ellipse class="cls-1" cx="271.34" cy="481.08" rx="41.16" ry="39.8" />
        <text class="cls-4" transform="translate(259.11 495.87) scale(.97 1)">
          <tspan x="0" y="0">6</tspan>
        </text>
        <ellipse class="cls-1" cx="273.57" cy="372.42" rx="41.16" ry="39.8" />
        <text class="cls-4" transform="translate(261.34 387.53) scale(.97 1)">
          <tspan x="0" y="0">9</tspan>
        </text>
        <ellipse class="cls-5" cx="382.97" cy="481.83" rx="42.58" ry="40.72" />
        <text class="cls-2" transform="translate(356.1 493.85) scale(.98 1)">
          <tspan x="0" y="0">מחק</tspan>
        </text>
        <ellipse class="cls-5" cx="382.17" cy="376.34" rx="42.58" ry="40.72" />
        <text class="cls-3" transform="translate(367.73 393.04) scale(.98 1)">
          <tspan x="0" y="0">=</tspan>
        </text>
        <rect class="cls-6" x="82.42" y="197.43" width="69.41" height="151.42" rx="17.78" ry="17.78" transform="translate(-156.02 390.26) rotate(-90)" />
        <text class="cls-2" transform="translate(56.51 281.72) scale(1.06 1)">
          <tspan x="0" y="0">סוג קרקע</tspan>
        </text>
        <rect class="cls-5" x="298.22" y="599.02" width="170.8" height="96" rx="29.2" ry="29.2" transform="translate(-263.4 1030.64) rotate(-90)" />
        <text class="cls-2" transform="translate(350.9 635.54) scale(1.12 1)">
          <tspan x="0" y="0">שינוי</tspan>
          <tspan x="6.87" y="39.6">זוית</tspan>
        </text>
        <rect class="cls-6" x="299.8" y="197.75" width="69.41" height="151.42" rx="17.78" ry="17.78" transform="translate(61.05 607.96) rotate(-90)" />
        <text class="cls-2" transform="translate(281.08 282.75) scale(1.06 1)">
          <tspan x="0" y="0">נוסחאות</tspan>
        </text>
      </g>
    </svg>
</template>
  
<script>
  export default {
    name: "CustomSvgComponent",
  };
</script>
  
 <style scoped>
  .cls-1 {
    fill: #4d4d4d;
  }
  .cls-2 {
    font-family: OpenSansSemiCondensed-SemiBoldItalic, 'Open Sans SemiCondensed';
    font-size: 33px;
    font-style: italic;
    font-weight: 600;
  }
  .cls-2,
  .cls-3,
  .cls-4 {
    fill: #fff;
  }
  .cls-5 {
    fill: #b3b3b3;
  }
  .cls-3 {
    font-size: 55px;
  }
  .cls-3,
  .cls-4 {
    font-family: Heebo-Regular, Heebo;
  }
  .cls-4 {
    font-size: 45px;
  }
  .cls-6 {
    fill: #fbb03b;
  }
</style>
  